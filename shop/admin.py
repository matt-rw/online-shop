...
from .models import (
    Color, Product, ProductVariant, Size,
    EmailSubscription, EmailTemplate, EmailCampaign, EmailLog,
    SMSSubscription, SMSTemplate, SMSCampaign, SMSLog,
    SiteSettings,
...
class EmailSubscriptionAdmin(admin.ModelAdmin):
    ...
    def export_as_csv(self, request, queryset):
        ...
        response['Content-Disposition'] = f'attachment; filename="email_subscribers_{datetime.now().strftime("%Y%m%d")}.csv"'
...
class EmailTemplateAdmin(admin.ModelAdmin):
    ...
    list_display = ('name', 'template_type', 'auto_trigger', 'is_active', 'times_used', 'updated_at')
...
class EmailCampaignAdmin(admin.ModelAdmin):
    ...
    readonly_fields = ('total_recipients', 'sent_count', 'failed_count', 'started_at', 'completed_at', 'created_at', 'created_by')
    ...
    fieldsets = (
        ...
        ('Targeting', {
            'fields': ('send_to_all_active',)
        }),
        ('Scheduling', {
            'fields': ('scheduled_at',)
        }),
        ('Progress', {
            'fields': ('total_recipients', 'sent_count', 'failed_count', 'started_at', 'completed_at'),
            'classes': ('collapse',)
        }),
        ('Notes', {
    ...
    def progress(self, obj):
        ...
        if obj.total_recipients == 0:
            return '-'
        percentage = (obj.sent_count / obj.total_recipients) * 100 if obj.total_recipients > 0 else 0
        return format_html(
            '{}/{} ({}%)',
    ...
    def send_now(self, request, queryset):
        ...
        for campaign in queryset:
            if campaign.status in ['draft', 'scheduled']:
                ...
                if 'error' in result:
                    self.message_user(request, f'Campaign {campaign.name} failed: {result["error"]}', level='ERROR')
                else:
                    self.message_user(request, f'Campaign {campaign.name} completed. Sent: {result["sent"]}, Failed: {result["failed"]}')
            else:
                self.message_user(request, f'Campaign {campaign.name} cannot be sent (status: {campaign.status})', level='WARNING')
...
class EmailLogAdmin(admin.ModelAdmin):
    ...
    readonly_fields = ('subscription', 'email_address', 'subject', 'html_body', 'text_body',
                      'campaign', 'template', 'status', 'sent_at', 'delivered_at', 'error_message')
...
class SMSSubscriptionAdmin(admin.ModelAdmin):
    ...
    def export_as_csv(self, request, queryset):
        ...
        response = HttpResponse(content_type='text/csv')
        response['Content-Disposition'] = f'attachment; filename="sms_subscribers_{datetime.now().strftime("%Y%m%d")}.csv"'
...
class SMSLogAdmin(admin.ModelAdmin):
    ...
    readonly_fields = ('subscription', 'phone_number', 'message_body', 'campaign', 'template',
                      'status', 'twilio_sid', 'sent_at', 'delivered_at', 'error_message', 'cost')
    date_hierarchy = 'sent_at'
...
class PageViewAdmin(admin.ModelAdmin):
    ...
    readonly_fields = ('path', 'method', 'ip_address', 'user_agent', 'referrer', 'referrer_domain',
                      'device_type', 'browser', 'os', 'viewed_at', 'response_time_ms', 'session_id')
...
class VisitorSessionAdmin(admin.ModelAdmin):
    ...
    list_display = ('session_id_short', 'page_views', 'device_type', 'landing_page', 'first_seen', 'last_seen')
    list_filter = ('device_type', 'first_seen')
    search_fields = ('session_id', 'ip_address', 'landing_page')
    readonly_fields = ('session_id', 'first_seen', 'landing_page', 'referrer', 'last_seen',
                      'page_views', 'ip_address', 'user_agent', 'device_type')
...
class CampaignAdmin(admin.ModelAdmin):
    ...
    list_display = ("name", "status", "target_group", "total_messages", "sent_messages", "created_at")
    list_filter = ("status", "created_at")
    search_fields = ("name", "description", "target_group")
    readonly_fields = ("created_at", "updated_at", "started_at", "completed_at", "total_messages", "sent_messages")
    inlines = [CampaignMessageInline]

    fieldsets = (
        ("Campaign Details", {
            "fields": ("name", "description", "status", "target_group")
        }),
        ("Timestamps", {
            "fields": ("created_at", "updated_at", "started_at", "completed_at")
        }),
        ("Stats", {
            "fields": ("total_messages", "sent_messages")
        }),
    )
